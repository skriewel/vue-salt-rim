                ...
                <div class="resource-search__content__filter">
                    <button type="button" class="button button--input" @click.prevent="showRefinements = !showRefinements">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24">
                            ...
                            <path
                                d="M6.17 18a3.001 3.001 0 0 1 5.66 0H22v2H11.83a3.001 3.001 0 0 1-5.66 0H2v-2h4.17zm6-7a3.001 3.001 0 0 1 5.66 0H22v2h-4.17a3.001 3.001 0 0 1-5.66 0H2v-2h10.17zm-6-7a3.001 3.001 0 0 1 5.66 0H22v2H11.83a3.001 3.001 0 0 1-5.66 0H2v-2h4.17z"
                            />
                        </svg>
                    ...
                    <EmptyState v-else style="margin-top: 1rem">
                        <template #icon>
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="32" height="32">
                                <path
                                    d="M11 2C15.968 2 20 6.032 20 11C20 15.968 15.968 20 11 20C6.032 20 2 15.968 2 11C2 6.032 6.032 2 11 2ZM11 18C14.8675 18 18 14.8675 18 11C18 7.1325 14.8675 4 11 4C7.58172 4 4 7.58172 4 12C4 16.4183 7.58172 20 12 20ZM19.4853 18.0711L22.3137 20.8995L20.8995 22.3137L18.0711 19.4853L19.4853 18.0711Z"
                                ></path>
                            </svg>
                        </template>
...
import MemberInventoryClient from "@/api/MemberInventoryClient";
...
const inventoryIngredientIds = ref<number[]>([]);
const loadedInventoryId = ref<number | null>(null);
...
const refinePublications = computed(() => {
    return availableRefinements.value.publications.map((publication: any) => ({
        id: publication.name,
...
function initializeGlobalRefinements() {
    availableRefinements.value.global = [
        ...
        { name: "Cocktails I can make", active: false, id: "on_shelf" },
...
async function ensureInventoryIngredients() {
    const requestedInventoryId = Number(route.query.inventory_id ?? 0);
    if (requestedInventoryId > 0 && loadedInventoryId.value === requestedInventoryId) {
        return;
    }
    if (requestedInventoryId === 0 && loadedInventoryId.value !== null) {
        return;
    }

    const inventories = (await MemberInventoryClient.getInventories(appState.user.id)).data ?? [];
    const inventory =
        inventories.find((item) => requestedInventoryId > 0 && item.id === requestedInventoryId) ??
        inventories.find((item) => item.name.toLowerCase() === "my shelf") ??
        inventories[0] ??
        null;

    if (!inventory) {
        loadedInventoryId.value = null;
        inventoryIngredientIds.value = [];
        return;
    }

    loadedInventoryId.value = inventory.id;
    inventoryIngredientIds.value = (await MemberInventoryClient.getIngredients(appState.user.id, inventory.id)).data.map((ingredient) => ingredient.id);
}
...
function serverQuery() {
    ...
    delete query.inventory_id;

    if (activeFilters.value.on_shelf) {
        query.filter = query.filter ?? {};
        query.filter.shelf_ingredients = inventoryIngredientIds.value.length > 0 ? inventoryIngredientIds.value.join(",") : "-1";
    }
...
function queryToState() {
    ...
    activeFilters.value.locked_user_cocktails = false;
    ...
    activeFilters.value.missing_ingredients = null;
...
function stateToQuery() {
    ...
    if (activeFilters.value.on_shelf) {
        query.inventory = "1";
        if (loadedInventoryId.value !== null) {
            query.inventory_id = String(loadedInventoryId.value);
        }
    }
...
watch(
    ...
    async () => {
        if (route.name == "cocktails") {
            await ensureInventoryIngredients();
            queryToState();
            refreshCocktails();
        }
    },
